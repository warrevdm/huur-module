...
{
    return find_reservation($reservationId);
}
...
function create_contract_for_reservation(int $reservationId): array
{
    ...
    if (empty($reservation['bikes'])) {
        throw new RuntimeException('Er zijn geen fietsen aan deze verhuur gekoppeld.');
    }
    ...
    $_SESSION['contract_tokens'][$contractId] = $token;
    audit('create', 'rental_contract', $contractId, ['reservation_id' => $reservationId, 'bike_count' => count($reservation['bikes'])]);
...
function render_contract_document(array $reservation, string $number, ?array $signature): string
{
    ...
    $totalDailyRate = array_sum(array_map(
    ...
    $lateFee = $totalDailyRate > 0 ? '€ ' . number_format($totalDailyRate, 2, ',', '.') : 'de op dat moment geldende gezamenlijke daghuur';

    $bikeRows = '';
    foreach ((array) $reservation['bikes'] as $bike) {
        $details = [e((string) $bike['category'])];
        if (!empty($bike['frame_size'])) {
            $details[] = 'maat ' . e((string) $bike['frame_size']);
        }
        if (!empty($bike['frame_number'])) {
            $details[] = 'framenummer ' . e((string) $bike['frame_number']);
        }
        $rate = (float) ($bike['reserved_daily_rate'] ?? $bike['daily_rate'] ?? 0);
        $bikeRows .= '<tr><td><strong>' . e((string) $bike['code']) . ' — ' . e((string) $bike['name']) . '</strong><br><span class="meta">' . implode(' · ', $details) . '</span></td><td>€ ' . number_format($rate, 2, ',', '.') . '</td></tr>';
    }
    ...
    if ($signature) {
        ...
        $signedBlock = '<div class="signature-box">'
            ...
            . '<p class="small">Bewijsreferentie: ' . e((string) $signature['signed_hash']) . '</p></div>';
    } else {
        $signedBlock = '<div class="signature-box"><p><strong>Elektronische ondertekening volgt via de ondertekenpagina.</strong></p></div>';
    }

    $companyExtra = '';
    if ($company['phone'] !== '') $companyExtra .= ' · ' . e((string) $company['phone']);
    if ($company['vat'] !== '') $companyExtra .= ' · ' . e((string) $company['vat']);
    $plural = count((array) $reservation['bikes']) === 1 ? 'fiets' : 'fietsen';

    return '<!doctype html><html lang="nl-BE"><head><meta charset="utf-8"><style>'
        . '@page{margin:24mm 18mm}body{font-family:DejaVu Sans,Arial,sans-serif;color:#17211a;font-size:10pt;line-height:1.42}'
        . 'h1{font-size:20pt;margin:0 0 4px}h2{font-size:12.5pt;margin:18px 0 7px;border-bottom:1px solid #dce4dd;padding-bottom:4px}'
        . '.brand{color:#31852c;font-weight:700}.meta{color:#657167;font-size:8.7pt}.grid{width:100%;border-collapse:collapse;margin:8px 0}'
        . '.grid th,.grid td{border:1px solid #dce4dd;padding:7px;text-align:left;vertical-align:top}.grid th{width:29%;background:#f5f8f5}'
        . '.bikes th:first-child{width:auto}.bikes th:last-child,.bikes td:last-child{width:22%;white-space:nowrap}'
        . 'ol{padding-left:20px}li{margin-bottom:6px}.signature-box{margin-top:20px;border:1px solid #8ca08f;border-radius:8px;padding:14px;background:#f8fbf8}'
        ...
        . '.notice{padding:10px;border-left:4px solid #60bb46;background:#f1f8ef}</style></head><body>'
        . '<div class="brand">' . e((string) $company['name']) . '</div>'
        . '<div class="meta">' . e((string) $company['address']) . ' · ' . e((string) $company['email']) . $companyExtra . '</div>'
        . '<h1>Huurovereenkomst ' . e($plural) . '</h1><p class="meta">Contractnummer ' . e($number) . '</p>'
        . '<h2>1. Partijen en reservatie</h2><table class="grid">'
        ...
        . '<tr><th>Totale huurprijs</th><td>€ ' . number_format($totalPrice, 2, ',', '.') . '</td></tr></table>'
        . '<h2>2. Gehuurde ' . e($plural) . '</h2><table class="grid bikes"><tr><th>Fiets</th><th>Dagprijs</th></tr>' . $bikeRows . '</table>'
        . '<h2>3. Staat, gebruik en veiligheid</h2><ol>'
        . '<li>De verhuurder levert de fietsen in een veilige, bruikbare en bij afhaling gezamenlijk controleerbare staat. Zichtbare opmerkingen worden vóór vertrek genoteerd.</li>'
        . '<li>De huurder gebruikt elke fiets zorgvuldig, overeenkomstig de bestemming, de verkeersregels en de gegeven gebruiksinstructies. Onderverhuur of terbeschikkingstelling aan derden is niet toegestaan zonder voorafgaand akkoord.</li>'
        . '<li>De huurder beveiligt elke fiets bij onbeheerde stalling met het meegeleverde of een gelijkwaardig degelijk slot. Sleutels, laders en accessoires blijven bij de juiste fiets.</li>'
        . '<li>Normale slijtage is voor rekening van de verhuurder. Schade door foutief, roekeloos of onzorgvuldig gebruik wordt aangerekend op basis van de aantoonbare herstelkost.</li></ol>'
        . '<h2>4. Teruggave, laattijdigheid en niet-teruggave</h2><ol>'
        . '<li>Alle fietsen moeten uiterlijk op het afgesproken eindmoment en op de afgesproken plaats worden teruggebracht, tenzij vooraf schriftelijk een verlenging is bevestigd.</li>'
        . '<li>Bij laattijdige teruggave zonder voorafgaand akkoord is per begonnen periode van 24 uur een bijkomende gebruiksvergoeding verschuldigd van ' . e($lateFee) . '. Een hogere vergoeding kan alleen worden gevraagd voor aantoonbare bijkomende schade.</li>'
        . '<li>Wanneer een fiets na schriftelijke ingebrekestelling niet wordt teruggebracht, is de huurder aansprakelijk voor de aantoonbare actuele vervangingswaarde, rekening houdend met leeftijd, staat en normale slijtage, plus redelijke bewezen recuperatiekosten.</li>'
        . '<li>Bij diefstal of verlies verwittigt de huurder de verhuurder onmiddellijk en doet zo snel mogelijk aangifte bij de politie. Het proces-verbaal en alle beschikbare sleutels en accessoires worden bezorgd.</li></ol>'
        . '<h2>5. Verplichtingen van de verhuurder</h2><ol>'
        . '<li>Indien een gereserveerde fiets niet beschikbaar of niet veilig bruikbaar is, biedt de verhuurder een gelijkwaardig alternatief zonder toeslag, een nieuwe datum of terugbetaling van het niet-uitgevoerde deel.</li>'
        . '<li>De verhuurder beperkt zijn aansprakelijkheid niet voor schade uit bedrog, zware fout of een wettelijke verplichting die niet contractueel kan worden uitgesloten.</li></ol>'
        . '<h2>6. Persoonsgegevens en elektronische ondertekening</h2><ol>'
        ...
        . '<li>De elektronische ondertekening wordt gekoppeld aan dit contractnummer en de ongewijzigde contractinhoud. Datum, tijdstip, technische bewijsgegevens en een cryptografische hash worden bewaard.</li>'
        . '<li>De huurder ontvangt na ondertekening een kopie via het opgegeven e-mailadres.</li></ol>'
        . '<div class="notice"><strong>Akkoordverklaring.</strong> De huurder verklaart alle fietsen en accessoires te hebben gecontroleerd, de bepalingen te hebben gelezen en ermee akkoord te gaan.</div>'
        . $signedBlock . '</body></html>';
}
...
{
    if (!empty($contract['signed_at'])) return $contract;
    ...
    $signature = ['signer_name' => $signerName, 'signed_at' => $signedAt->format('Y-m-d H:i:s'), 'signature_data_uri' => $normalizedDataUri, 'signed_hash' => ''];
    $preHashHtml = render_contract_document($reservation, (string) $contract['contract_number'], $signature);
    $signedHash = hash('sha256', $preHashHtml . '|' . (string) $contract['contract_hash']);
    $signature['signed_hash'] = $signedHash;
    $signedHtml = render_contract_document($reservation, (string) $contract['contract_number'], $signature);
    ...
    try {
        ...
        if ($stmt->rowCount() !== 1) throw new RuntimeException('Dit contract werd intussen al ondertekend.');
        db()->commit();
    } catch (Throwable $e) {
    ...
    $stmt = db()->prepare('UPDATE rental_contracts SET email_status=:status, email_sent_at=:sent_at, email_error=:error, updated_at=CURRENT_TIMESTAMP WHERE id=:id');
    $stmt->execute([
    ...
    audit('sign', 'rental_contract', $contractId, ['reservation_id' => (int) $contract['reservation_id'], 'email_status' => $mailResult['status'], 'signed_hash' => $signedHash]);
    return find_contract_by_id($contractId) ?? $updated;
}

function generate_contract_pdf(array $contract): ?string
{
    if (empty($contract['signed_contract_html']) || !class_exists(\Dompdf\Dompdf::class)) return null;
    ...
    $path = $dir . '/' . $name;
    if (file_put_contents($path, $dompdf->output(), LOCK_EX) === false) return null;
    @chmod($path, 0640);
    return $name;
}
