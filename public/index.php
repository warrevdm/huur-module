...
if ($route === 'planning') {
    ...
    <section class="card mt-18">
        <div class="planning-toolbar">
            ...
            <div class="actions"><a href="?route=planning&amp;days=7">7 dagen</a><a href="?route=planning&amp;days=14">14 dagen</a><a href="?route=planning&amp;days=28">28 dagen</a><a class="button" href="?route=reservation-new">+ Nieuwe verhuur</a></div>
        </div>
        <?php if (!$bikes): ?>
            ...
            <?php foreach ($bikes as $bike): $cursor = 0; $bikeEvents = $byBike[(int) $bike['id']] ?? []; ?>
                ...
                <?php while ($cursor < $days):
                    ...
                    if ($active):
                        ...
                        $span = 1;
                        while ($cursor + $span < $days && $start->modify('+' . ($cursor + $span) . ' days') < $activeEnd) { $span++; }
                ?>
...
if ($route === 'bikes') {
    if ($method === 'POST') {
        ...
        if (!$data[':code'] || !$data[':name'] || !$data[':category']) {
            flash('error', 'Code, naam en categorie zijn verplicht.'); redirect('?route=bikes');
        }
        try {
            $stmt = db()->prepare('INSERT INTO bikes (code,name,category,frame_size,daily_rate,deposit,status,notes) VALUES (:code,:name,:category,:frame_size,:daily_rate,:deposit,:status,:notes)');
            $stmt->execute($data);
    ...
    <section class="grid">
        <div class="card col-8"><h2>Overzicht</h2><div class="table-wrap"><table><thead><tr><th>Code</th><th>Fiets</th><th>Categorie</th><th>Maat</th><th>Dagprijs</th><th>Waarborg</th><th>Status</th></tr></thead><tbody>
        <?php foreach ($bikes as $bike): ?><tr><td><strong><?= e($bike['code']) ?></strong></td><td><?= e($bike['name']) ?></td><td><?= e($bike['category']) ?></td><td><?= e($bike['frame_size'] ?: '—') ?></td><td>€ <?= number_format((float) $bike['daily_rate'], 2, ',', '.') ?></td><td>€ <?= number_format((float) $bike['deposit'], 2, ',', '.') ?></td><td><span class="badge badge-<?= e($bike['status']) ?>"><?= e(ucfirst($bike['status'])) ?></span></td></tr><?php endforeach; ?>
        <?php if (!$bikes): ?><tr><td colspan="7" class="muted">Nog geen fietsen.</td></tr><?php endif; ?>
        </tbody></table></div></div>
        <div class="card col-4"><h2>Fiets toevoegen</h2><form method="post" class="stack"><input type="hidden" name="_token" value="<?= e(csrf_token()) ?>">
            ...
            <div class="form-grid"><div class="field"><label>Dagprijs</label><input name="daily_rate" type="number" min="0" step="0.01" value="0"></div><div class="field"><label>Waarborg</label><input name="deposit" type="number" min="0" step="0.01" value="0"></div></div>
            ...
            <div class="field"><label>Notities</label><textarea name="notes"></textarea></div><button class="button">Fiets opslaan</button>
        </form></div>
    </section>
    <?php render_footer(); exit;
}
...
if ($route === 'reservation-new') {
    ...
    if ($method === 'POST') {
        ...
        if (!$bikeId || !$startAt || !$endAt || $endAt <= $startAt || !$name) {
            flash('error', 'Vul alle verplichte velden correct in.'); redirect('?route=reservation-new');
        }
        if (reservation_conflicts($bikeId, $startAt->format('Y-m-d H:i:s'), $endAt->format('Y-m-d H:i:s'))) {
            flash('error', 'Deze fiets is al ingepland binnen dit tijdsblok.'); redirect('?route=reservation-new&amp;bike_id=' . $bikeId . '&amp;start_date=' . $startAt->format('Y-m-d'));
        }
        try {
            db()->beginTransaction();
            $stmt = db()->prepare('INSERT INTO customers (name,email,phone,address) VALUES (:name,:email,:phone,:address)');
            $stmt->execute([':name'=>$name, ':email'=>trim((string)($_POST['customer_email']??''))?:null, ':phone'=>trim((string)($_POST['customer_phone']??''))?:null, ':address'=>trim((string)($_POST['customer_address']??''))?:null]);
            ...
            $stmt = db()->prepare('INSERT INTO reservations (bike_id,customer_id,identity_document_id,start_at,end_at,status,total_price,deposit_amount,deposit_paid,notes,created_by) VALUES (:bike,:customer,:document,:start,:end,:status,:price,:deposit,:paid,:notes,:user)');
            $stmt->execute([':bike'=>$bikeId, ':customer'=>$customerId, ':document'=>$documentId, ':start'=>$startAt->format('Y-m-d H:i:s'), ':end'=>$endAt->format('Y-m-d H:i:s'), ':status'=>($_POST['status']??'reserved')==='confirmed'?'confirmed':'reserved', ':price'=>max(0,(float)($_POST['total_price']??0)), ':deposit'=>max(0,(float)($_POST['deposit_amount']??0)), ':paid'=>isset($_POST['deposit_paid'])?1:0, ':notes'=>trim((string)($_POST['notes']??''))?:null, ':user'=>current_user()['id']]);
            $id = (int) db()->lastInsertId();
            db()->commit();
            audit('create', 'reservation', $id, ['bike_id'=>$bikeId, 'has_identity_document'=>$documentId!==null]);
            flash('success', 'Verhuur is ingepland.'); redirect('?route=reservation-view&amp;id=' . $id);
        } catch (Throwable $e) {
            if (db()->inTransaction()) db()->rollBack();
            flash('error', $e instanceof RuntimeException ? $e->getMessage() : 'Opslaan is mislukt.'); redirect('?route=reservation-new');
        }
    }
    $bike = $selectedBike ? find_bike($selectedBike) : null;
    ...
    <section class="card"><form method="post" enctype="multipart/form-data" class="stack"><input type="hidden" name="_token" value="<?= e(csrf_token()) ?>">
        <div class="form-grid">
            ...
            <div class="field"><label>Startdatum *</label><input name="start_date" type="date" value="<?= e($startDate) ?>" required></div><div class="field"><label>Afhaaluur *</label><input name="start_time" type="time" value="09:00" required></div>
            <div class="field"><label>Einddatum *</label><input name="end_date" type="date" value="<?= e($endDate) ?>" required></div><div class="field"><label>Retouruur *</label><input name="end_time" type="time" value="17:00" required></div>
        </div><hr><h2>Klantgegevens</h2><div class="form-grid">
            <div class="field"><label>Volledige naam *</label><input name="customer_name" required></div><div class="field"><label>Telefoon</label><input name="customer_phone" type="tel"></div>
            <div class="field"><label>E-mail</label><input name="customer_email" type="email"></div><div class="field"><label>Adres</label><input name="customer_address"></div>
        ...
        </div><hr><h2>Financieel en intern</h2><div class="form-grid">
            <div class="field"><label>Status</label><select name="status"><option value="reserved">Gereserveerd</option><option value="confirmed">Bevestigd</option></select></div><div class="field"><label>Totaalprijs</label><input name="total_price" type="number" min="0" step="0.01" value="0"></div>
            <div class="field"><label>Waarborg</label><input name="deposit_amount" type="number" min="0" step="0.01" value="<?= e($bike ? (string)$bike['deposit'] : '0') ?>"></div><div class="field"><label><input class="checkbox-inline" name="deposit_paid" type="checkbox" value="1"> Waarborg ontvangen</label></div>
            <div class="field field-full"><label>Interne notities</label><textarea name="notes"></textarea></div>
        </div><div class="actions"><button class="button">Verhuur inplannen</button><a class="button button-secondary" href="?route=planning">Annuleren</a></div>
    </form></section>
    <?php render_footer(); exit;
}
...
if ($route === 'reservation-view') {
    ...
    if (!$reservation) { http_response_code(404); exit('Verhuur niet gevonden.'); }
    ...
    <section class="grid">
        <div class="card col-8"><div class="actions actions-between"><h2><?= e($reservation['bike_code'].' — '.$reservation['bike_name']) ?></h2><span class="badge status-<?= e($reservation['status']) ?>"><?= e(status_label($reservation['status'])) ?></span></div>
            <dl class="summary-list"><dt>Periode</dt><dd><?= e((new DateTimeImmutable($reservation['start_at']))->format('d/m/Y H:i')) ?> → <?= e((new DateTimeImmutable($reservation['end_at']))->format('d/m/Y H:i')) ?></dd><dt>Klant</dt><dd><?= e($reservation['customer_name']) ?></dd><dt>Telefoon</dt><dd><?= e($reservation['customer_phone']?:'—') ?></dd><dt>E-mail</dt><dd><?= e($reservation['customer_email']?:'—') ?></dd><dt>Adres</dt><dd><?= e($reservation['customer_address']?:'—') ?></dd><dt>Totaalprijs</dt><dd>€ <?= number_format((float)$reservation['total_price'],2,',','.') ?></dd><dt>Waarborg</dt><dd>€ <?= number_format((float)$reservation['deposit_amount'],2,',','.') ?> · <?= $reservation['deposit_paid']?'ontvangen':'niet ontvangen' ?></dd><dt>Notities</dt><dd><?= nl2br(e($reservation['notes']?:'—')) ?></dd></dl>
        </div>
        <div class="card col-4"><h2>Identiteitsdocument</h2>
        <?php if ($reservation['document_id'] && !$reservation['document_deleted_at']): ?><p><strong><?= e($reservation['document_name']) ?></strong><br><span class="muted"><?= e($reservation['document_mime']) ?> · <?= number_format((int)$reservation['document_size']/1024,0,',','.') ?> KB</span></p><p class="muted">Bewaren tot <?= e($reservation['retention_until'] ? (new DateTimeImmutable($reservation['retention_until']))->format('d/m/Y') : 'niet ingesteld') ?></p><a class="button" href="?route=id-download&amp;id=<?= (int)$reservation['document_id'] ?>">Veilig openen</a>
        <?php else: ?><p class="muted">Nog geen document gekoppeld.</p><form method="post" action="?route=id-upload&amp;reservation_id=<?= $id ?>" enctype="multipart/form-data" class="stack"><input type="hidden" name="_token" value="<?= e(csrf_token()) ?>"><div class="field"><label>Document</label><input name="identity_document" type="file" accept="image/jpeg,image/png,application/pdf" required></div><div class="field"><label>Verwijderen na</label><input name="retention_until" type="date" value="<?= e((new DateTimeImmutable($reservation['end_at']))->modify('+' . (int)env('ID_RETENTION_DAYS','30') . ' days')->format('Y-m-d')) ?>" required></div><button class="button">ID veilig uploaden</button></form><?php endif; ?>
        </div>
...
if ($route === 'reservation-status' && $method === 'POST') {
    ...
    if (!find_reservation($id) || !in_array($status, ['reserved','confirmed','picked_up','returned','cancelled'], true)) { flash('error','Ongeldige status.'); redirect('?route=planning'); }
    ...
    flash('success','Status aangepast.'); redirect('?route=reservation-view&amp;id='.$id);
}
...
if ($route === 'id-upload' && $method === 'POST') {
    ...
    $reservation = find_reservation($id);
    if (!$reservation) { flash('error','Verhuur niet gevonden.'); redirect('?route=planning'); }
    try {
        $documentId = upload_identity_document($_FILES['identity_document'] ?? [], (int)$reservation['customer_id'], (string)($_POST['retention_until']??''));
        if (!$documentId) throw new RuntimeException('Selecteer een document.');
    ...
    } catch (Throwable $e) { flash('error', $e instanceof RuntimeException ? $e->getMessage() : 'Uploaden is mislukt.'); }
...
if ($route === 'id-download') {
    ...
    $stmt->execute([':id'=>$id]);
    $document = $stmt->fetch();
    if (!$document) { http_response_code(404); exit('Document niet gevonden.'); }
    $path = ROOT_PATH . '/storage/private/ids/' . basename($document['stored_name']);
    if (!is_file($path)) { http_response_code(404); exit('Bestand niet gevonden.'); }
